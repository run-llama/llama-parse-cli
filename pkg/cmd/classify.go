...
var classifyDelete = cli.Command{
	Name:    "delete",
	Usage:   "Delete a classify job and its result.",
	Suggest: true,
	Flags: []cli.Flag{
		&requestflag.Flag[string]{
			Name:      "job-id",
...
func handleClassifyDelete(ctx context.Context, cmd *cli.Command) error {
	client := llamacloud.NewClient(getDefaultRequestOptions(cmd)...)
	unusedArgs := cmd.Args().Slice()
	...
	params := llamacloud.ClassifyDeleteParams{}
