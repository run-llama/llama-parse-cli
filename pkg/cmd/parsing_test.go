...
func TestParsingCreate(t *testing.T) {
	...
	t.Run("inner flags", func(t *testing.T) {
		...
		mocktest.TestRunMockTestWithFlags(
			...
			"--output-options.granular-bboxes", "[word, line, cell]",
			"--output-options.images-to-save", "[embedded]",
			"--output-options.markdown", "{annotate_line_numbers: true, annotate_links: true, annotate_revisions: true, inline_images: true, tables: {compact_markdown_tables: true, markdown_table_multiline_separator: markdown_table_multiline_separator, merge_continued_tables: true, output_tables_as_markdown: true}}",
			"--output-options.save-output-pdf=true",
...
func TestParsingCancel(t *testing.T) {
	t.Skip("Mock server tests are disabled")
	t.Run("regular flags", func(t *testing.T) {
		mocktest.TestRunMockTestWithFlags(
			t,
