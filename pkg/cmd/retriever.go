...
var retrieversListPaginated = cli.Command{
	Name:    "list-paginated",
	Usage:   "List the retrievers in a project, newest first.",
	Suggest: true,
	Flags: []cli.Flag{
	...
	Action:          handleRetrieversListPaginated,
	HideHelpCommand: true,
}
...
func handleRetrieversListPaginated(ctx context.Context, cmd *cli.Command) error {
	client := llamacloud.NewClient(getDefaultRequestOptions(cmd)...)
	unusedArgs := cmd.Args().Slice()
	...
	params := llamacloud.RetrieverListPaginatedParams{}
	...
	if format == "raw" {
		var res []byte
		options = append(options, option.WithResponseBodyInto(&res))
		_, err = client.Retrievers.ListPaginated(ctx, params, options...)
		if err != nil {
			return err
		}
		obj := gjson.ParseBytes(res)
		return ShowJSON(obj, ShowJSONOpts{
			...
			Format:         format,
			RawOutput:      cmd.Root().Bool("raw-output"),
			Title:          "retrievers list-paginated",
			Transform:      transform,
		})
	} else {
		iter := client.Retrievers.ListPaginatedAutoPaging(ctx, params, options...)
		maxItems := int64(-1)
